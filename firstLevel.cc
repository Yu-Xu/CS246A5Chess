...
//This method will return a random move by the rules but won't know whether there are pieces in the way that prevent it
		...
		switch(randomNumber)
		{
			...
			case 2:
			{
				...
				if(numOfRooks > 0)
				{
					...
					toMoveFrom = rooks.at(distRookSize(rng)); //Choose a random rook to move
			...
			case 3:
			{
				...
				if(numOfBishops > 0)
				{
					...
					toMoveFrom = bishops.at(distBishopSize(rng)); //Choose a random bishop to move
					std::uniform_int_distribution<std::mt19937::result_type> dist7(1,7);
					std::uniform_int_distribution<std::mt19937::result_type> dist4(1,4);
					int newRandomNum = dist7(rng); //Choose how many squares to move
					int randomDirection = dist4(rng); //Choose a direction
			...
			case 4:
			{
				...
				if(numOfKnights > 0)
				{
					...
					toMoveFrom = knights.at(distKnightSize(rng)); //Choose a random knight to move
					std::uniform_int_distribution<std::mt19937::result_type> dist4(1,4);
					std::uniform_int_distribution<std::mt19937::result_type> dist2(1,2);
