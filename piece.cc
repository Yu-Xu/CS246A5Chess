...
Piece::Piece():
	subject{nullptr}, colour{0},
	location{std::pair<int,int>{-1, -1}}, empty{1} {
		std::cout << "ARE WE HERE" << std::endl;
}

Piece::Piece(ChessBoard* subject, bool colour, int row, int col, bool empty):
	subject{subject}, colour{colour},
	location{std::pair<int,int>{row,col}}, empty{empty} {}
...
bool Piece::notify() {
	...
	//Case where this is the piece we want to move
	if(moveFrom == pieceLocation) {
    	if(this->isEmpty()) {
	...
	//Cases where the current piece is not the one being moved but might be in the way of the move
	} else {
		if(this->isEmpty()) {
			return true;
		}
